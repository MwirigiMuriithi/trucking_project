...
const TripForm: React.FC<TripFormProps> = ({ setTripData }) => {
  const [driverName, setDriverName] = useState<string>(''); // New field for driver name
  const [driverId, setDriverId] = useState<string>('');       // Optional field for existing driver ID
  const [currentLocation, setCurrentLocation] = useState<string>('');
  const [pickupLocation, setPickupLocation] = useState<string>('');
  const [dropoffLocation, setDropoffLocation] = useState<string>('');
  const [currentCycleUsed, setCurrentCycleUsed] = useState<string>(''); // Managed by backend, but included for testing
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      // Construct the payload including driverName and driverId.
      // Backend can use driverName to create a new driver if driverId is not provided.
      const payload = {
        driverName,
        driverId,
        currentLocation,
      ...
      const response = await axios.post<TripData>(`${API_ROOT}/api/calculate-trip/`, payload);
      setTripData(response.data);
    } catch (err: any) {
      console.error("Error calculating trip:", err.response?.data || err.message);
      setError("Failed to calculate trip. Please check your inputs.");
  ...
  return (
    <div className="bg-white shadow rounded p-6 mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-gray-700">Driver ID (if available)</label>
          <input
            type="text"
            value={driverId}
